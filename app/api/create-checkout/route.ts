...
const PRICES: Record<string, { amount: number; interval: "month" | "year"; name: string }> = {
  pro_monthly:      { amount: 1200,  interval: "month", name: "DealPilot Pro (Monthly)" },
  pro_yearly:       { amount: 10800, interval: "year",  name: "DealPilot Pro (Yearly — save 25%)" },
  business_monthly: { amount: 2900,  interval: "month", name: "DealPilot Business (Monthly)" },
  business_yearly:  { amount: 26400, interval: "year",  name: "DealPilot Business (Yearly — save 24%)" },
};
...
export async function POST(req: NextRequest) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        currency: "usd",
        product_data: { name: priceConfig.name, description: "DealPilot subscription" },
